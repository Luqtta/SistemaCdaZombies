...
public class ZombieTransformListener implements Listener {
    ...
    public void onInteract(PlayerInteractEntityEvent e) {
        ...
        if (rule.displayNameContains != null && !rule.displayNameContains.trim().isEmpty()) {
            String need = rule.displayNameContains.trim().toLowerCase();
            ...
            if (!display.toLowerCase().contains(need)) return;
        }

        LivingEntity target = (LivingEntity) e.getRightClicked();

        // Só transforma se for um zombie nosso
        if (!target.hasMetadata(ZombieManager.META_ZOMBIE_KEY)) return;
