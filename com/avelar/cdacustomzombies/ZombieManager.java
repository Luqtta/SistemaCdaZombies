...
public class ZombieManager {
    ...
    // raw spawn weights (para permitir multiplicadores em eventos)
    ...
    // marcas de explosões do explosivo (para bloquear quebra de blocos)
    ...
    private void loadTransformItems() {
        ...
        for (String idStr : ti.getKeys(false)) {
            try {
                ...
                transformRules.put(id, new TransformRule(id, to.toUpperCase(), consume, contains));
            } catch (Throwable ignored) {}
        }
    ...
    private void applyFlameVisual(org.bukkit.entity.LivingEntity le, String mode) {
        ...
        if ("FIRETICKS".equalsIgnoreCase(mode)) {
            try {
                le.setFireTicks(40); // mantém visual; pode causar dano se o servidor aplicar
            } catch (Throwable ignored) {}
            return;
        }
	...
	// chamado periodicamente pelo Main
	public void tickTerrorClock() {
	    if (!terrorEnabled) {
	        // se desativado por config, limpa estado
	        terrorForcedActive = null;
	        terrorActive = false;
	        terrorLastReminderMillis = 0L;
	        return;
        ...
        if (desired != terrorActive) {
            ...
            if (msg != null && msg.length() > 0) {
                // Remove eventual prefix como [MADRUGADA] da mensagem antes de broadcast
                String cleaned = stripMadrugadaPrefix(msg);
                // Mensagens podem ser multilinha (use \n no config ou bloco YAML "|")
    ...
    public void processZombieDeath(LivingEntity mob, Player killerParam) {
        ...
        int baseGiven = giveItemsWithChance(killer, def.getItems(), getTerrorRareChanceMultiplier());
        ...
        if (boss || lend) {
            ...
            if (reward != null && Math.random() <= reward.chance) {
                ...
                bonusGiven = giveItemsSimple(killer, reward.items, key); // reward items = 100% (sem chance)
            }
    ...
    private int giveItemsWithChance(Player p, List<String> items, double chanceMultiplier) {
        ...
        for (String s : items) {
            try {
                ...
                p.getInventory().addItem(new ItemStack(id, amount, (short) 0, data));
                given++;
            } catch (Throwable ignored) {}
        }
    ...
    public static class LevelRule {
        ...
        public TransformRule(int itemId, String toZombieKey, boolean consume, String displayNameContains) {
            this.itemId = itemId;
